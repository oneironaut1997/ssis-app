		...
		<div class="width--100">
			...
			<v-row class="pa-6">
                <v-col cols="12" class="pa-0 messages-container">
					<template v-for="(message, index) in ictmo_messages.messages">
		                <div
					...
					<template v-if="ictmo_messages.length < 1">
...
export default {
    ...
    methods: {
    	...
    	fetch() {
        	axios.post(this.routes['api.user.fetch-ictmo-messenger'], {user_id : this.user.id})
            .then(response => {
                const data = response.data;

		        this.$store.commit('ictmo_messages/set', data.ictmo_messages);
		        this.setItem('ictmo_messages', data.ictmo_messages);

		        this.ictmo_messages = this.getItem('ictmo_messages');
        ...
        submit() {
            ...
            payloads.append('room_id', this.ictmo_messages.id ? this.ictmo_messages.id : '');

            axios.post(this.routes['api.user.ictmo-messenger'], payloads)
            .then(response => {
            	this.fetch();
