		...
		<div class="width--100">
			...
			<v-row class="pa-6">
                <v-col cols="12" class="pa-0 messages-container">
					<template v-for="(message, index) in registrar_messages.messages">
		                <div
					...
					<template v-if="registrar_messages.length < 1">
...
export default {
    ...
    methods: {
    	...
    	fetch() {
        	axios.post(this.routes['api.user.fetch-registrar-messenger'], {user_id : this.user.id})
            .then(response => {
                const data = response.data;

		        this.$store.commit('registrar_messages/set', data.registrar_messages);
		        this.setItem('registrar_messages', data.registrar_messages);

		        this.registrar_messages = this.getItem('registrar_messages');
        ...
        submit() {
            ...
            payloads.append('room_id', this.registrar_messages.id ? this.registrar_messages.id : '');
