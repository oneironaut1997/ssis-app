		...
		<div class="width--100">
			...
			<v-row class="pa-6">
                <v-col cols="12" class="pa-0 messages-container">
					<template v-for="(message, index) in cashier_messages.messages">
		                <div
					...
					<template v-if="cashier_messages.length < 1">
...
export default {
    ...
    methods: {
    	...
    	fetch() {
        	axios.post(this.routes['api.user.fetch-cashier-messenger'], {user_id : this.user.id})
            .then(response => {
                const data = response.data;

		        this.$store.commit('cashier_messages/set', data.cashier_messages);
		        this.setItem('cashier_messages', data.cashier_messages);

		        this.cashier_messages = this.getItem('cashier_messages');
    	...
    	listen() {
        	window.Echo.channel('admin-chat.'+this.cashier_messages.id)
	        .listen('.admin.chat-received', (e) => {
	            console.log(e.chatMessage);
	            console.log("receiving message...")
	            this.cashier_messages.messages.push(e.chatMessage);
	        })
    	},
        ...
        submit() {
            ...
            payloads.append('room_id', this.cashier_messages.id ? this.cashier_messages.id : '');

            axios.post(this.routes['api.user.cashier-messenger'], payloads)
            .then(response => {
            	this.fetch();
